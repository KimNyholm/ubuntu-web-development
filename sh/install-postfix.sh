...
sudo debconf-set-selections <<< 'postfix postfix/mailname string ubuntu.test'
sudo apt-get --yes install postfix

#Install mail stack delivery.
sudo debconf-set-selections <<< 'dovecot-core dovecot-core/ssl-cert-name string ubuntu.test'
